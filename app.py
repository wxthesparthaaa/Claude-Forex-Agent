...
DEVELOPER_NOTES = [
    ...
    ("2026-09-12", "Friday reflection's 'Confidence weight reassessment' section now says '(base strategy "
                    ...
                    "Label-only change, no behavior affected."),
    ...
    ("2026-09-09", "Found the REAL cause of VWAP Scalp's same-tick trade clustering (yesterday's fix wasn't "
                    ...
                    "the cooldown. `now` now refreshes every pair; the guard also no longer inverts on a "
                    "negative gap."),
    ...
    ("2026-08-30", "Trader-book round 3, candidate #1 (Fibonacci retracement pullback): 1682 signals (highest "
                    "count all session), best horizon (20d) closest yet to significance but still misses raw "
                    "p=0.05 (p=0.0739). Ruled out. Moving to Nison candlestick reversal patterns next."),
    ("2026-08-30", "Trader-book round 2, candidate #3 (Oops gap reversal -- Larry Williams): 524 signals, "
                    "nothing survives even raw p<0.05 at any of 5 horizons. Ruled out -- CLOSES round 2 (Turtle "
                    "Soup, Bollinger squeeze, Oops gap). 21 hypotheses tested this session total, none survived."),
    ...
    ("2026-08-30", "Trader-book candidate #3 (NFP event reaction) ruled out -- closes the ENTIRE trader-book "
                    "thread. 4h/24h both survived Bonferroni together (most consistent result all session) but "
                    "split-half dissolved both. 18 total hypotheses tested this session, none survived."),
    ...
    ("2026-08-29", "Carry+momentum 'fix' for CHF_JPY was trend-following in disguise (proved via an unconstrained "
                    "trend test beating it on 12/12 pairs incl. one with no carry side at all). Not shipped -- but "
                    "surfaces pure trend-following (Sharpe 1.35, 13/13 pairs robust) as its own lead worth vetting."),
    ...
    ("2026-08-29", "Carry trade (Ledger #3): first genuinely positive result all session. JPY-funded crosses "
                    "(USD_JPY +45%, GBP_JPY +47%, EUR_JPY +43% price-only over ~8yr) track real yen-weakness "
                    "history -- but it's one historical era, and non-JPY pairs' 'today's rate' caveat bites hard."),
